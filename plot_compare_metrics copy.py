...
def _annotate_stats(ax, data, fontsize=9):
    if data.size == 0:
        return
    mu = np.nanmean(data)
    ...
    label = f"N={n}, mean={mu:.3g}, median={med:.3g}"
    ax.text(
        0.02, 0.98,              # canto superior esquerdo (em coords do eixo)
        label,
...
def scatter_across_scenarios(scen_data: dict, x_metric: str, y_metric: str,
    ...
    labels = {"normal": "Scenario 1 — Nominal",
              "delay": "Scenario 2 — Delay",
              "wrong_model": "Scenario 3 — Perturbed "}
    plotted = False
    for scen, df in scen_data.items():
...
def box_per_metric(metric: str, scen_data: dict, out_dir: Path, violin=False,
    ...
    if set_common_ylim:
        ...
        lo = 0
        hi = np.percentile(all_vals, 99.5) * 1.05
        ax.set_ylim(lo, hi)

    # anota N acima de cada caixa
    ymin, ymax = ax.get_ylim()
    offset = (ymax - ymin) * 0.03  # 3% da altura

    for i, arr in enumerate(data, start=1):
        ax.text(
            i,
            ymax - offset,          # um pouco abaixo do topo
            f"N={len(arr)}",
            ha="center",
            va="top",
            fontsize=8,
