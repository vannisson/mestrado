...
# —————————————————————————————————————————————
# Variáveis globais e preparação de diretório
# —————————————————————————————————————————————
self = type('', (), {})()
...
FORCE_DELAY_START  = 10.0    # 10 s
FORCE_DELAY_END    = 15.0    # 15 s
FORCE_DELAY_VALUE  = 0.5     # 500 ms

# instante de início (para cálculo de elapsed)
start_time = time.time()

# —————————————————————————————————————————————
# Função de logging de CSV — já está no escopo global
# —————————————————————————————————————————————
...
def on_message(client, userdata, msg):
    # não bloqueia aqui: só armazena pendente
    if msg.topic == "agv/odometry/wheel_vel":
        odom = OdometryData()
        odom.ParseFromString(msg.payload)
        if len(odom.wheel_velocities) >= 2:
            self._pending = (time.time(), tuple(odom.wheel_velocities))

# —————————————————————————————————————————————
# SysCalls do script de CoppeliaSim
# —————————————————————————————————————————————
def sysCall_init():
    sim = require('sim')
    ...
    self._pending           = None  # tupla (t_recv, (vL, vR))

    # cria client MQTT (sem callback_api_version)
...
def sysCall_actuation():
    ...
    # se houver mensagem pendente, aplica após delay adequado
    # if self._pending is not None:
    #     t_recv, (vl, vr) = self._pending
    #     elapsed_from_recv = time.time() - t_recv
    #     elapsed_from_start = t_recv - start_time

    #     # escolhe delay conforme janela forçada
    #     if FORCE_DELAY_START <= elapsed_from_start <= FORCE_DELAY_END:
    #         desired_delay = FORCE_DELAY_VALUE
    #     else:
    #         desired_delay = BASE_DELAY

    #     if elapsed_from_recv >= desired_delay:
    #         self.received_left_vel, self.received_right_vel = vl, vr
    #         self._pending = None

    # aplica velocidades no robô
...
def sysCall_sensing():
    ...
    left_vel    = sim.getJointVelocity(self.motorLeft)
    right_vel   = sim.getJointVelocity(self.motorRight)
    ...
    odom_msg = OdometryData(
        ...
        wheel_velocities=[left_vel, right_vel],
    )
    ...
    log_data("odometry/wheel_vel",   [left_vel, right_vel])

def sysCall_cleanup():
    # encerra loop de rede e desconecta, se existir
